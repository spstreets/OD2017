...
bike_paths = st_read(file.path(folder, "SAD69-96_SHP_redecicloviaria/SAD69-96_SHP_redecicloviaria.shp"))
  
st_crs(bike_paths) = 5533     

bike_paths = st_transform(bike_paths,
                          crs = 4326)
...
od_zones = st_read(file.path(folder, "shapefiles/Zonas_2017_region.shp")) %>%
  filter(NomeMunici == "São Paulo") %>%
  st_buffer(dist = 0) %>%
  st_transform(4326) %>%
  st_make_valid()

sw <- st_read(file.path(folder,"Poligonos_calcadas/Calcadas_SP.shp")) %>%
  st_buffer(dist = 0) %>%
  st_transform(4326) %>%
  st_make_valid()

streets <- st_read(file.path(folder,"Poligonos_vias/Poligonos_vias_total.gpkg")) %>%
  st_transform(4326)
