...
We can look at the number of trips recorded in different days of the week as follows:
...
od_hour = OD_2017_v1 %>% 
  ...
  summarise(
    n_trips = n(),
    mean_weight = mean(fe_via),
    total_trips = n_trips * mean_weight
    )

od_hour_arrival = OD_2017_v1 %>% 
  group_by(h_cheg) %>% 
  summarise(
    n_trips = n(),
    mean_weight = mean(fe_via),
...
od_hour %>% 
  ggplot() +
  geom_line(aes(h_saida, total_trips)) +
  geom_line(aes(h_cheg, total_trips), data = od_hour_arrival, color = "blue") +
  geom_vline(xintercept = c(10, 15))

```

Looking at the same graph but taking into account the minutes (i.e. using hour as a continuous variable)
...
OD_2017_v1 %>%
  ...
  ggplot() +
  geom_density(aes(x=saida, weight=fe_via)) +
  geom_density(aes(x=chegada, weight=fe_via), color="blue") +
  geom_vline(xintercept = c(10, 15))
