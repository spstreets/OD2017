...
folder = "C:/Users/Lucas/Downloads/OD2017/"
...
```{r, include=FALSE}
zonas_od = st_read(file.path(folder, "shapefiles/Zonas_2017_region.shp"))
...
tm_shape(origin) + tm_fill("trips_o", 
                           style = "cont",
                           title = "Trips' origins")  +
...
tm_shape(destination) + tm_fill("trips_d", 
                                style = "cont",
...
origin %>%
  ...
  tm_layout(title = "São Paulo City",
            inner.margins = c(0, .02, .02, .02),
            legend.outside = TRUE)
...
bike_pairs_inter = bike_pairs %>%   
  filter(zona_o != zona_d) %>%
  left_join(zonas_centroids, by = c("zona_o" = "NumeroZona")) %>%
  rename(origin = geometry) %>%
  left_join(zonas_centroids, by = c("zona_d" = "NumeroZona")) %>%
  rename(destination = geometry) %>%
  mutate(route = NA)
  

# alguma coisa errada aqui, o plot é muito estranho!

for (i in 1:dim(bike_pairs_inter)[1]) {
      
  bike_pairs_inter[i, "route"] = osrmRoute(src = st_as_sf(bike_pairs_inter[i, "origin"]),
                                           dst = st_as_sf(bike_pairs_inter[i, "destination"]),
                                           overview = "simplified",
                                           returnclass = "sf",
                                           osrm.profile = "bike") %>%
                                 select(geometry)
...
bike_routes = bike_pairs_inter %>%
  select(-origin, -destination) %>%
  st_as_sf()

saveRDS(bike_routes, file = "bike_routes_2021-09-07.Rds")
...
for (i in 1:dim(car_pairs_inter)[1]) {
      
  car_pairs_inter[i, "route"] = osrmRoute(src = st_as_sf(car_pairs_inter[i, "origin"]),
                                          dst = st_as_sf(car_pairs_inter[i, "destination"]),
                                          overview = "simplified",
                                          returnclass = "sf",
                                          osrm.profile = "car") %>%
                                 select(geometry)
...
car_routes = car_pairs_inter %>%
  select(-origin, -destination) %>%
  st_as_sf()

saveRDS(car_routes, file = "car_routes_2021-09-07.Rds")
...
for (i in 1:dim(foot_pairs_inter)[1]) {
      
  foot_pairs_inter[i, "route"] = osrmRoute(src = st_as_sf(foot_pairs_inter[i, "origin"]),
                                           dst = st_as_sf(foot_pairs_inter[i, "destination"]),
                                           overview = "simplified",
                                           returnclass = "sf",
                                           osrm.profile = "foot") %>%
                                 select(geometry)
...
foot_routes = foot_pairs_inter %>%
  select(-origin, -destination) %>%
  st_as_sf()

saveRDS(foot_routes, file = "foot_routes_2021-09-07.Rds")
...
bike_routes = readRDS("bike_routes_2021-09-07.Rds")
foot_routes = readRDS("foot_routes_2021-09-07.Rds")
car_routes = readRDS("car_routes_2021-09-07.Rds")

zonas_wgs = zonas_od %>%
  st_buffer(dist = 0) %>%              # without this step tmap_mode("view") does not work
  st_transform(crs = "WGS84")
...
leaflet(route_net_bike) %>% 
  ...
  addPolylines(weight = route_net_bike$bike / 100000, color = "white")
...
leaflet(route_net_car) %>% 
  addProviderTiles(providers$CartoDB.DarkMatter) %>%
  addPolylines(weight = route_net_car$car / 100000, color = "white")
...
leaflet(route_net_foot) %>% 
  addProviderTiles(providers$CartoDB.DarkMatter) %>%
  addPolylines(weight = route_net_foot$foot / 100000, color = "white")
